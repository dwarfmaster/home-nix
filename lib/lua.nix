...
let
  ...
  in types.coercedTo types.lines mkCode sub;

  mkFunction = args: body: let
    bd = if body ? __raw then body.__raw else body;
  in mkCode ''
  ...
  toLuaObject = args: let
      inherit (lib) concatStringsSep mapAttrsToList stringToCharacters filterAttrs
        boolToString concatMapStringsSep head escapeShellArg;
